...
const BookTurf = ({ Data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await BookingPost(TakingInfo, tokenData?.token);
    if (result.insertedId != null) {
      toast.success(
        "Your Request Is pending. Please Cheak my booking page if it is accepted by owner or not.",
      );
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 bg-slate-50 rounded-3xl border border-gray-100 shadow-sm mt-6">
      <div className="mb-8 text-center md:text-left">
        <h1 className="text-2xl md:text-3xl font-extrabold text-[#3b3bf3] tracking-tight">
          Book The Ground
        </h1>
        <p className="text-sm text-gray-500 mt-1">
          Secure your slot instantly. Select your preferred date, timings, and
          play!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                {Data.time_slots.map((res, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {res.isBooked ? (
                        <span className="text-[10px] font-bold bg-gray-200 text-gray-500 px-2 py-0.5 rounded">
                          Booked
                        </span>
                      ) : isSelected ? (
                        <svg
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-gray-100 shadow-xl shadow-slate-100/50 flex flex-col justify-between min-h-[350px]">
          <div>
            <h2 className="text-lg font-bold text-gray-900 pb-3 border-b border-gray-100 mb-5">
